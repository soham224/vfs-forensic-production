...
import {getChairOccupancyCount, getResult, getResultById} from "../../_redux/VFSAPI";
...
function OccupancyDashboard() {
    ...
    const [occupancyDataResponse, setOccupancyDataResponse] = useState(null);
    ...
    useEffect(() => {
        const crowdResultData = {
            ...
            end_date: endDate,
            label: ["occupancy_detection"],
            time_zone: "asia/kolkata",
            duration_type: "day",
            get_id: false,
        };
        fetchResults(crowdResultData, setIsOccupancyLoading, setOccupancyDataResponse);
        const data = {
            start_date: startDate,
            end_date: endDate
        };
    ...
    const handleApplied = () => {
        ...
        fetchResults(resultData, setIsOccupancyLoading, setOccupancyDataResponse);
    ...
    const onDrilldown = async (e) => {
        ...
        if (name.includes("-")) {
            ...
            const drilldownData = response.data.occupancy_detection.map(item => ({
                name: item.name,
                y: item.y,
                id: item.id,
        ...
        <>
            ...
            <Grid container spacing={2} mt={2}>
                ...
                <Grid item xl={6} lg={6} md={6} sm={12} xs={12}>
                    ...
                    <App
                        title="Occupancy Graph By Time"
                        count={''}
                        seriesData={occupancyDataResponse && occupancyDataResponse?.occupancy_detection ? occupancyDataResponse?.occupancy_detection : []}
                        chartType={"column"}
                        chartLoading={isOccupancyLoading}
                        noDataMessage={"No data found"}
                        setStartDate={setStartDate}
                        setEndDate={setEndDate}
                        chartName={'occupancy_detection'}
                        setIsOccupancyLoading={setIsOccupancyLoading}
                        setOccupancyDataResponse={setOccupancyDataResponse}
